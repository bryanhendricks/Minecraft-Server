...
REMOTE_FOLDER_PATH="/minecraft/bettermc/world" # Folder to zip on the remote server
...
ssh "$REMOTE_USER@$REMOTE_HOST" "sudo systemctl stop $REMOTE_SERVICE_NAME"
if [ $? -ne 0 ]; then
...
ssh "$REMOTE_USER@$REMOTE_HOST" "zip -r /tmp/$ZIP_FILE_NAME $REMOTE_FOLDER_PATH"
if [ $? -ne 0 ]; then
...
scp "$REMOTE_USER@$REMOTE_HOST:/tmp/$ZIP_FILE_NAME" "$LOCAL_DOWNLOAD_PATH"
if [ $? -ne 0 ]; then
...
ssh "$REMOTE_USER@$REMOTE_HOST" "sudo systemctl start $REMOTE_SERVICE_NAME"
if [ $? -ne 0 ]; then
...
ssh "$REMOTE_USER@$REMOTE_HOST" "rm /tmp/$ZIP_FILE_NAME"
if [ $? -ne 0 ]; then
